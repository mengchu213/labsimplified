# spec/requests/admin/registrations_request_spec.rb

require 'rails_helper'

RSpec.describe "Admin::Registrations", type: :request do
  let(:admin) { create(:user, role: :admin) }
  let(:user) { create(:user, role: :medtech) }
  ...
  describe "PATCH /update" do
    context "when logged in as admin" do
      ...
      context "with valid parameters" do
        ...
        it "updates the user" do
          patch user_registration_path(user.id), params: { user: new_attributes }
          user.reload
      ...
      context "with invalid parameters" do
        it "does not update the user" do
          patch user_registration_path(user.id), params: { user: { name: "" } }
          user.reload
    ...
    context "when not logged in as admin" do
      ...
      it "redirects to the dashboard with an alert" do
        patch user_registration_path(user.id), params: { user: { name: "Updated Name" } }
        expect(response).to redirect_to(dashboard_path)
      end
    end
  end
  ...
  describe "DELETE /destroy" do
    ...
    context "when not logged in as admin" do
      ...
      it "redirects to the dashboard with an alert" do
        delete destroy_user_admin_path(user)
        expect(response).to redirect_to(dashboard_path)
